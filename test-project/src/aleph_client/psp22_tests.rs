...
use anyhow::Result;
use assert2::assert;
use ink_wrapper_types::{util::ToAccountId, Connection as _, SignedConnection as _, TxStatus};
...
async fn connect_and_deploy() -> Result<(SignedConnection, psp22_contract::Instance)> {
    ...
    let contract = conn
        .instantiate(
            psp22_contract::Instance::new(1000).with_salt(salt),
            TxStatus::Finalized,
        )
        .await?;
...
async fn test_transfers() -> Result<()> {
    ...
    conn.exec(
        contract.transfer(other_account_id.into(), 100, vec![]),
        TxStatus::Finalized,
    )
    .await?;
