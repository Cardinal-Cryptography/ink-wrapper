...
pub trait UploadConnection<TxInfo, E>: Sync {
    ...
    fn upload(&self, _call: UploadCall) -> Result<TxInfo, E> {
        unimplemented!()
    }
}
...
pub trait SignedConnection<TxInfo, E>: Sync {
    ...
    fn instantiate_tx<T: Send + From<AccountId>>(
        &self,
    ...
    fn instantiate<T: Send + From<AccountId>>(&self, call: InstantiateCall<T>) -> Result<T, E> {
        let (contract, _) = self.instantiate_tx(call)?;
        Ok(contract)
    }

    /// Perform the given mutating call.
    fn exec(&self, call: ExecCall) -> Result<TxInfo, E>;
}
...
pub trait Connection<TxInfo, E>: Sync {
    ...
    fn read<T: scale::Decode + Send>(&self, call: ReadCall<T>) -> Result<T, E>;

    /// Fetch all events emitted by contracts in the transaction with the given `tx_info`.
    fn get_contract_events(&self, tx_info: TxInfo) -> Result<ContractEvents, E>;
}
